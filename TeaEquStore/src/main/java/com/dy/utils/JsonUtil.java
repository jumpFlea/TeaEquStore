...
public static final Logger logger = LoggerFactory.getLogger(JsonUtil.class);
    ...
    /*public static void sendErrorJson(HttpServletResponse response, String msg, Exception e)
        ...
        //输出错误参数的原因
        if( e != null && e instanceof Assert.ParamException){
        	msg += "\t" + e.getMessage();
        }
        result.put("success", false);
        result.put("info", msg);
        if(msg.contains("该用户已在其它设备登录")){
        	result.put("isLoginOther", 1);
        	result.put("info", "该用户已在其它设备登录");
        }
        sendJson(response, result.toString());
    }
    */
   ...
   /* public static void putData(JSONObject result, Page page){
    ...
    /*public static List<PropertyFilter> getFilters(HttpServletRequest request){
    	...
    	for(Entry<String, String[]> entry: set){
    		...
    		if(name.toUpperCase().startsWith("FILTER_") && StringUtils.isNotEmpty(entry.getValue()[0])){
    			name = StringUtils.substringAfter(name, "_");
    			filters.add(new PropertyFilter(name,entry.getValue()[0]));
    		}
    	}
    	return filters;
    }*/
